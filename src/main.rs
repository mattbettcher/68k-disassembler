...
fn decode_size(size: u16) -> &'static str {
    match size {
        8 => ".b",
        16 => ".w",
        32 => ".l",
...
// requires up to 6 extension words, these must be in ext
fn decode_ea(op: u16, ext: Vec<u16>, size: u16) -> String {
    // extract extension word info
    let d_or_a = (ext[0] >> 15) & 1 == 1;
    let xr = (ext[0] >> 12) & 7;
    let w_or_l = (ext[0] >> 11) & 1 == 1;
    let scale = (ext[0] >> 9) & 3;
    let disp = (ext[0] & 0xf) as i8;
    ...
    match (op >> 3) & 0x7 {
        ...
        0b011 => format!("(A{})+", op & 7),
        0b100 => format!("-(A{})", op & 7),
        0b101 => format!("({},A{})", ext[0] as i16, op & 7),
        0b110 => format!("Nope"),
        0b111 => {
            match op & 7 {
				0b000 => format!("({:X}).W", ext[0]),
				0b001 => format!("({:X}).L", ((ext[0] as u32) << 16) + ext[1] as u32),
                0b010 => format!("({}, PC)", ext[0] as i16),
				0b011 => format!("({},PC,{}{})", disp, "D", xr),	// TODO(matt) this isn't locked to D
				0b100 => {
                    match size {
                        8 => format!("#{:X}", ext[0] & 0xf),
                        16 => format!("#{:X}", ext[0]),
                        32 => format!("#{:X}", ((ext[0] as u32) << 16) + ext[1] as u32),
                        _ => unimplemented!(), 
                    }
                }
                _ => unimplemented!(),
...
fn main() {
    ...
    let inst = Inst {
        ...
        aea: b"A+-DXWLdxI",
        cycles: [8, 8, 2],
        bitp: Regex::new(r"1101[0-1]{3}011(010|011|100|101|110|111)[0-1]{3}").unwrap(),
    };
    // let inst = Inst { name: "adda",     size: 16, sp: SP::n,   se: SE::a,    	aea: b"..........", cycles: [ 8,  8,  2],	bitp: Regex::new(r"1101[0-1]{3}011001[0-1]{3}").unwrap()};
    // let inst = Inst { name: "addx",     size:  8, sp: SP::rr,  se: SE::n,    	aea: b"..........", cycles: [ 4,  4,  2],	bitp: Regex::new(r"1101[0-1]{3}100000[0-1]{3}").unwrap()};

    for i in 0..0xffff {
        if inst.bitp.is_match(format!("{:b}", i).as_str()) {
            println!("{:X}\t{}{} {}",
                     i,
                     inst.name,
                     decode_size(inst.size),
                     decode_ea(i, vec![0xFF82, 0xABCD, 0], inst.size).as_str());
        }
    }
